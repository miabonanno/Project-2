...
def find_urls(s):
    lst =[]
    lst = re.findall("\w*(http[s]?://(?:\w*[.]\w{2,})+)", s)
    return lst
...
def grab_headlines():
    ...
    div_containing_answer = soup.find("div", {"class": "view-most-read"}).findChildren("a") # list of all child a elements
    list_of_headlines = []
    for i in div_containing_answer:
         little_soup = str(i)
         little_soup = BeautifulSoup(little_soup, "html.parser")
         list_of_headlines.append(little_soup.get_text())
...
def get_umsi_data():
    umsi_titles = {}
    r2 = requests.get("https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All", headers={"User-Agent": 'SI_CLASS'})
    ...
    for x in range(13):
        ...
        directory = requests.get(updated_url, headers={'User-Agent': 'SI_CLASS'})
        new_directory = r2.text + directory.text

    soup2 = BeautifulSoup(new_directory, 'html.parser')


    titles_list= soup2.find_all("div", class_="field field-name-field-person-titles field-type-text field-label-hidden")
    names_list= soup2.find_all("div", class_="field field-name-title field-type-ds field-label-hidden")

    total = 0
    for z in names_list:
        umsi_titles[z.text]=titles_list[total].text
        total += 1
